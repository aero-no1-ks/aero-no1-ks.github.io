...
<v-layout row wrap class='pa-1'>
  <v-flex xs12 class='px-1'>
    ...
    <v-menu offset-y>
      ...
      <v-list>
        <v-list-tile v-for='(x, i) in ["Shrink", ...oriArrFull]' :key='i' @click='large = i - 1'>
    ...
    <v-layout row wrap>
      <template v-for='(x, i) in items'>
        <v-flex :xs4='!isLarge(i)' :xs12='isLarge(i)' class='pa-1' :key='i' :style='setShow(i)'>
          <v-card dark>
            <app-map-pane :style='largeStyle(i)' :center='getCenter(center[i])' :items='x' :map='map' :large='large' :slider='slider' :xycor='xycor' />
          </v-card>
        </v-flex>
      </template>
    </v-layout>
...
export default {
  data: () => ({
    ...
    tooltipItems: [
      ...
      { title: 'To destination', icon: 'looks_one', description: 'direct flights' },
      { title: 'To hub', icon: 'looks_two', description: 'to hub (1st stop of indirect flights)' },
    ]
  }),
  computed: {
    ...mapState(['oriArr', 'dataOri', 'airport']),
    ...mapGetters(['oriArrFull']),
    center() {
      return _.map(this.oriArr, n => _.find(this.dataOri, o => n === o.ap))
    },
    prop() {
    ...
    aps() {
      return _.uniq([...this.oriArr, ..._.map(_.flattenDeep(this.items), 'ap')])
    },
    xycor() {
