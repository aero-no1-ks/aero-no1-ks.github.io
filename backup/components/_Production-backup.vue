      ...
      <v-card dark style='position: relative;'>
        <v-layout row class='subheading pa-2'>
          <v-flex style='max-width: 350px;'>
            ...
            <span class='bold'>{{ flight[i] }} Departure from {{ ap.ap }}</span>
          </v-flex>
        ...
        <v-slide-y-transition>
          <app-map-pane-departure v-show='!hide[i]' :style='largeStyle(i)' :ap='ap' :item='x' :map='map' :large='large[i]' :slider='slider[i]' />
        </v-slide-y-transition>
    ...
    <v-flex v-if='!large[i]' xs4 class='pa-1' d-flex>
      <v-card dark class='text-xs-center'>
        <div class='subheading pa-2'>
          ...
          <span class='bold'>Top {{ cut }} Dest. 'FROM' {{ ap.ap }} ({{ flight[i] }})</span>
        </div>
        <v-divider />
        <v-slide-y-transition>
      ...
      <v-layout v-if='showHub[i]'>
        <v-flex :xs8='!largeHub[i]' :xs12='largeHub[i]' class='pa-1' d-flex>
          <v-card dark style='position: relative;'>
            ...
            <v-divider />
            <app-map-pane-intermediate style='width: 100%;' :style='largeStyleHub(i)' :ap='ap' :hub='hub' :map='map' :large='largeHub[i]' :slider='sliderHub[i]' />
        ...
        <v-flex v-if='!largeHub[i]' xs4 class='pa-1' d-flex>
          <v-card dark class='text-xs-center'>
            <div class='subheading pa-2'>
              <v-icon class='subheading bold vertical-middle mr-1'>navigation</v-icon>
              <span class='bold'>Top {{ cut }} Departures 'FROM' {{ hub.ap }}</span>
            </div>
            <v-divider />
...
import MapPaneDeparture    from '@/components/shared/MapPaneDeparture'
import MapPaneIntermediate from '@/components/shared/MapPaneDepartureIntermediate'
...
export default {
  ...
  computed: {
    ...mapState(['ori']),
    ap() {
      return datasetOri[this.apIx]
    },
    items() {
      const x = datasetOri[this.apIx]
      return [
  ...
  methods: {
    ...
    updateIx(x) {
      const ix = _.findIndex(datasetOri, n => n.ap === x)
  ...
  components: {
    'app-map-pane-departure'   : MapPaneDeparture,
